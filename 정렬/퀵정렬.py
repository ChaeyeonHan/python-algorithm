...
def quick_sort(array):
    if len(array) <= 1:  # 리스트가 하나 이하의 원소를 갖고있다면 종료
        return array

    pivot = array[0]
    tail = array[1:]  # 피벗을 제외한 리스트

    left_side = [x for x in tail if x <= pivot]  # 왼쪽부분, 피벗보다 큰거 찾기
    right_side = [x for x in tail if x > pivot]  # 오른쪽부분, 피벗보다 작은거 찾기

    # 왼쪽과 오른쪽 부분에서 각각 퀵정렬을 수행후, 전체 리스트를 반환
